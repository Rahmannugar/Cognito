...
export function MessageBubble({ role, content, timestamp, avatar }: MessageBubbleProps) {
        ...
        <div className={cn('flex gap-3', isUser && 'flex-row-reverse')}>
            ...
            >
                {isUser ? (
                    ...
                    <img src={avatar} alt="Ajibade" className="w-full h-full object-cover" />
                ) : (
            ...
            <div className={cn('flex flex-col gap-1 max-w-[85%]', isUser && 'items-end')}>
                {timestamp && (
                    <span className={cn('text-xs text-gray-500', isUser ? 'mr-1' : 'ml-1')}>
                        {!isUser && 'Ajibade • '}
                        {timestamp}
                    </span>
                )}
